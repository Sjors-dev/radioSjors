...
| `news.*` | Off by default. `enabled: true` turns it on; `feeds` is a list of RSS URLs (BBC political + science by default, any well-formed RSS 2.0 feed works, swap in whatever you actually want). `dj.segments.news_chance` (default 0.4) is how often an hour gets one at all. `main.py news` shows exactly what the hosts would be given right now. |
